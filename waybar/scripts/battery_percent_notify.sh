...
# Only notify at 20, 10, or 5 percent
if [[ "$battery_percent" == "20" || "$battery_percent" == "10" || "$battery_percent" == "5" ]]; then
  last_notified=$(cat "$STATE_FILE" 2>/dev/null)
  if [ "$last_notified" != "$battery_percent" ]; then
    notify-send -t 10000 -u critical "Battery Low" "Your battery is at $battery_percent%!"
    echo "$battery_percent" >"$STATE_FILE"
  fi
else
  # Reset notification state if above 20
  if [ "$battery_percent" -gt 20 ]; then
    rm -f "$STATE_FILE"
  fi
fi
